...
void startPos()
{
  ...
  up_left = collisionMap[int(ghostA_X - myDoggy.doggyW/2)][int(ghostA_Y - myDoggy.doggyH/2)];
  up_right = collisionMap[int(ghostA_X + myDoggy.doggyW/2)][int(ghostA_Y - myDoggy.doggyH/2)];
  down_right = collisionMap[int(ghostA_X + myDoggy.doggyW/2)][int(ghostA_Y + myDoggy.doggyH/2)];
  down_left = collisionMap[int(ghostA_X - myDoggy.doggyW/2)][int(ghostA_Y + myDoggy.doggyH/2)];

  if(up_left==false || up_right==false || down_right==false || down_left==false || ghostA_X < kittyRefX+100 && ghostA_X > kittyRefX-100 && ghostA_Y < kittyRefY+100 && ghostA_Y > kittyRefY-100)
  {
    ghostA_X = int( random(100, width-100) );
    ghostA_Y = int( random(100, height-100) );
  }
